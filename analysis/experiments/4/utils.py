...
def detect_spikes_new(df, column, window_size, center=False):
    series = df[column]
    # Calculate rolling mean and std deviation for the current columnumn
    rolling_mean = series.rolling(window=window_size, center=center).mean().shift(-1)
    rolling_std = series.rolling(window=window_size, center=center).std().shift(-1)
    return (abs(series - rolling_mean) > SPIKES_THRESHOLD * rolling_std).astype(int)
