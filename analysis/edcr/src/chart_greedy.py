...
base_path0=sys.argv[1]
base_path1=sys.argv[2]
...
cla_datas = [cla0_data, cla1_data, cla2_data, cla3_data, cla4_data]
high_scores = [0.95, 0.98]
...
def ruleForNegativeCorrection(all_charts, epsilon):
    results = []
    total_results = np.copy(pred_data)
    for count, chart in enumerate(all_charts):
        chart = np.array(chart)
